...
function filterCategory(type: string) {
  ...
  if (type === "progress_updated" || type === "status_changed")
    return "progress";
...
export function ActivityTimeline({
  ...
  const filtered = React.useMemo(() => {
    ...
    const matching =
      filter === "all"
        ? sorted
        : sorted.filter((item) => filterCategory(item.type) === filter);
    return typeof limit === "number" ? matching.slice(0, limit) : matching;
  }, [activities, filter, limit]);

  const grouped = React.useMemo(
    () =>
      filtered.reduce<Record<string, ProjectActivity[]>>((groups, item) => {
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <Tooltip content="Displays your most recent project activity and progress across all projects.">
              <Info
                className="h-4 w-4 cursor-help text-slate-400 hover:text-indigo-400 transition-colors"
                aria-label="Activity timeline information"
              />
            </Tooltip>
          </div>
      ...
      {loading ? (
        <div className="space-y-3" aria-label="Loading project activity">
          {[0, 1, 2].map((item) => (
            <div
              key={item}
              className="flex animate-pulse gap-3 rounded-xl border border-white/5 bg-white/[0.02] p-4"
            >
        ...
        <div className="rounded-2xl border border-dashed border-white/10 bg-white/[0.02] px-6 py-12 text-center">
          <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-indigo-500/10 text-indigo-300">
          ...
          <p className="mx-auto mt-1 max-w-md text-xs leading-relaxed text-slate-400">
            Complete a milestone or initialize a project blueprint to create the
              ...
              <ol className="relative space-y-3 before:absolute before:bottom-5 before:left-5 before:top-5 before:w-px before:bg-white/10">
                {entries.map((item) => {
                  ...
                  return (
                    <li
                      key={item.id}
                      className="relative flex gap-3 rounded-xl border border-white/5 bg-white/[0.02] p-3.5 transition hover:border-indigo-400/20 hover:bg-indigo-500/[0.03]"
                    >
                      <div className="relative z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-indigo-400/15 bg-[#0e0a2a] text-indigo-300">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] text-slate-500">
                          {showProjectName && item.projectTitle ? (
                            <span className="truncate font-semibold text-indigo-300">
                              {item.projectTitle}
                            </span>
                          ) : null}
                          {showProjectName && item.projectTitle ? (
